    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div>
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold">Yêu cầu hỗ trợ</h2>
          <UButton color="primary" size="sm" @click="showNewTicket = !showNewTicket">
            Tạo yêu cầu mới
          </UButton>
        </div>

        <div v-if="showNewTicket" class="mb-6 p-4 border rounded-lg bg-gray-50">
          <div class="space-y-4">
            <div>
              <label class="text-sm font-medium mb-2 block">Tiêu đề</label>
              <UInput v-model="newTicket.title" placeholder="Nhập tiêu đề yêu cầu" />
            </div>
            <div>
              <label class="text-sm font-medium mb-2 block">Mô tả</label>
              <UTextarea v-model="newTicket.description" placeholder="Mô tả chi tiết vấn đề của bạn" />
            </div>
            <div>
              <label class="text-sm font-medium mb-2 block">Mức độ ưu tiên</label>
              <USelect v-model="newTicket.priority" :options="['low', 'medium', 'high']" />
            </div>
            <div class="flex gap-2">
              <UButton color="primary" @click="createTicket">Gửi yêu cầu</UButton>
              <UButton color="neutral" variant="outline" @click="showNewTicket = false">Hủy</UButton>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-start justify-between mb-2">
              ...
              <UBadge
                :color="ticket.status === 'resolved' ? 'green' : ticket.status === 'in-progress' ? 'blue' : 'orange'"
                size="sm"
              >
            ...
            <div class="flex items-center justify-between text-xs text-gray-500">
              ...
              <UBadge :color="ticket.priority === 'high' ? 'red' : ticket.priority === 'medium' ? 'yellow' : 'gray'" size="xs">
      ...
      <div>
        <h2 class="text-xl font-semibold mb-4">Câu hỏi thường gặp</h2>
        <div class="space-y-3">
          <UAccordion :items="faqs.map(faq => ({ label: faq.question, content: faq.answer }))" />
        </div>

        <div class="mt-6 p-4 bg-blue-50 rounded-lg">
          <h3 class="font-semibold mb-2">Liên hệ trực tiếp</h3>
