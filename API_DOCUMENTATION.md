...
```
- **Nghiệp vụ:** Thêm khóa học vào giỏ hàng
...
```
- **Nghiệp vụ:** Cập nhật số lượng trong giỏ hàng
...
- **Endpoint:** `GET /api/v1/cart/est-cart?cart_ids={cart_ids}&coupon_code={coupon}`
- **Headers:** `Authorization: Bearer {access_token}`
- **Query Parameters:**
  - `cart_ids`: Danh sách ID giỏ hàng (comma-separated)
...
```
- **Nghiệp vụ:** Tính toán tổng tiền và áp dụng khuyến mãi
- **Database:** Đọc từ `carts`, `courses`, `campaigns` table
...
```
- **Nghiệp vụ:** Mua khóa học ngay không qua giỏ hàng (sử dụng SePay)
